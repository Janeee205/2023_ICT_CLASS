...
document.querySelectorAll('.animate-input').forEach((e) => {
  ...
  // active라는 class를 추가해서 span이 위로 올라가게 한다.

  // .btn-login에 disabled라는 속성을 remove
  // else -> .animate-input에 active class remove span 원래자리로
  // btnLog disabled 속성 add
...
darkToggle.onclick = (e) => {
  ...
  // 삼항 연산자
  // 조건 ? 참일때 실행할 코드(true) : 거짓일때 실행할 코드(false)
