...
$('#veribtn').on('click', function () {
  ...
  if (phoneLeng && phoneNum) {
    $('.phone .warn').html('<span class="text-green"> 인증번호를 발송했습니다.(유효시간 30분) 인증번호가 오지 않으면 입력하신 정보가 정확한지 확인하여 주세요. 이미 가입된 번호이거나, 가상전화번호는 인증번호를 받을 수 없습니다. </span>');
...
// 행정자치부에서 제공하는 데이터 베이스를 바탕으로 업데이트
// api란 Application Programming Interface
// 프론트엔드(클라이언트), 백엔드(서버)가 요청과 응답을 주고 받을 수 있도록 만들어진 체계
...
// 가입하기 버튼을 눌렀을때
// #joinbtn click 했을 때 필수 요소가 모두 true라면(조건)
// #join-form submit()
...
$('#joinbtn').on('click', function (e) {
  if (idvari && pwveri && pwchkveri && nameveri && birthveri && genderveri && phoneveri && addrveri && mailveri) {
    $('#join-form').submit();
  } else {
    e.preventDefault()
